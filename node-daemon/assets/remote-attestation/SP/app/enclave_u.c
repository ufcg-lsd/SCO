...
static sgx_status_t SGX_CDECL enclave_emit_debug(void* pms)
{
...
static sgx_status_t SGX_CDECL enclave_print_byte_array(void* pms)
{
	ms_print_byte_array_t* ms = SGX_CAST(ms_print_byte_array_t*, pms);
	print_byte_array(ms->ms_mem, ms->ms_len);
...
static const struct {
	...
	11,
	{
		(void*)enclave_emit_debug,
		(void*)enclave_print_byte_array,
...
{
	sgx_status_t status;
	ms_enclave_set_ga_t ms;
	ms.ms_p_ga_in = p_ga_in;
	ms.ms_len = len;
...
{
	sgx_status_t status;
	ms_enclave_get_sign_gb_ga_t ms;
...
{
	sgx_status_t status;
	ms_enclave_compute_msg2_mac_t ms;
	ms.ms_p_src = p_src;
	ms.ms_len = len;
	ms.ms_p_mac_out = p_mac_out;

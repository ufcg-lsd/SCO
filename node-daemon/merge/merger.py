...
def create_response_bundle(dockerfile_dir, package, quote):
    ...
    with ZipFile(response_bundle_path, 'w') as response_bundle:
        ...
        print("[DEBUG] package filename IN BUNDLE is: " + package.filename)
        response_bundle.write(dockerfile_dir + "/" + package.filename)
...
def deploy_package(dir, package):
    path = os.path.join(dir, package.filename)
    print('[DEBUG] package filename is : ' + package.filename)
    package.save(path)

...
typedef struct ms_enclave_ra_init_t {
	...
	sgx_ec256_public_t* ms_p_gb_out;
} ms_enclave_ra_init_t;

typedef struct ms_enclave_set_ga_t {
	sgx_ec256_public_t* ms_p_ga_in;
	size_t ms_len;
} ms_enclave_set_ga_t;

typedef struct ms_enclave_derive_keys_t {
	sgx_status_t ms_retval;
} ms_enclave_derive_keys_t;

typedef struct ms_enclave_get_gb_t {
	sgx_status_t ms_retval;
	sgx_ec256_public_t* ms_p_gb_out;
} ms_enclave_get_gb_t;

typedef struct ms_enclave_get_sign_gb_ga_t {
	sgx_status_t ms_retval;
	sgx_ec256_signature_t* ms_p_sign_gb_ga;
} ms_enclave_get_sign_gb_ga_t;

typedef struct ms_enclave_compute_msg2_mac_t {
	sgx_status_t ms_retval;
	uint8_t* ms_p_src;
	size_t ms_len;
	sgx_cmac_128bit_key_t* ms_p_mac_out;
} ms_enclave_compute_msg2_mac_t;
...
typedef struct ms_print_byte_array_t {
	void* ms_mem;
	size_t ms_len;
} ms_print_byte_array_t;
...
static sgx_status_t SGX_CDECL sgx_enclave_ra_init(void* pms)
{
	...
	sgx_ec256_public_t* _tmp_p_gb_out = ms->ms_p_gb_out;
	size_t _len_p_gb_out = sizeof(*_tmp_p_gb_out);
	sgx_ec256_public_t* _in_p_gb_out = NULL;

	CHECK_REF_POINTER(pms, sizeof(ms_enclave_ra_init_t));
	CHECK_UNIQUE_POINTER(_tmp_p_gb_out, _len_p_gb_out);

	if (_tmp_p_gb_out != NULL) {
		if ((_in_p_gb_out = (sgx_ec256_public_t*)malloc(_len_p_gb_out)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_p_gb_out, 0, _len_p_gb_out);
	}
	ms->ms_retval = enclave_ra_init(_in_p_gb_out);
err:
	if (_in_p_gb_out) {
		memcpy(_tmp_p_gb_out, _in_p_gb_out, _len_p_gb_out);
		free(_in_p_gb_out);
	}
...
static sgx_status_t SGX_CDECL sgx_enclave_set_ga(void* pms)
{
	ms_enclave_set_ga_t* ms = SGX_CAST(ms_enclave_set_ga_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	sgx_ec256_public_t* _tmp_p_ga_in = ms->ms_p_ga_in;
	size_t _tmp_len = ms->ms_len;
	size_t _len_p_ga_in = _tmp_len;
	sgx_ec256_public_t* _in_p_ga_in = NULL;

	CHECK_REF_POINTER(pms, sizeof(ms_enclave_set_ga_t));
	CHECK_UNIQUE_POINTER(_tmp_p_ga_in, _len_p_ga_in);

	if (_tmp_p_ga_in != NULL) {
		_in_p_ga_in = (sgx_ec256_public_t*)malloc(_len_p_ga_in);
		if (_in_p_ga_in == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_p_ga_in, _tmp_p_ga_in, _len_p_ga_in);
	}
	enclave_set_ga(_in_p_ga_in, _tmp_len);
err:
	if (_in_p_ga_in) free(_in_p_ga_in);

	return status;
}

static sgx_status_t SGX_CDECL sgx_enclave_derive_keys(void* pms)
{
	ms_enclave_derive_keys_t* ms = SGX_CAST(ms_enclave_derive_keys_t*, pms);
	sgx_status_t status = SGX_SUCCESS;

	CHECK_REF_POINTER(pms, sizeof(ms_enclave_derive_keys_t));

	ms->ms_retval = enclave_derive_keys();


	return status;
}

static sgx_status_t SGX_CDECL sgx_enclave_get_gb(void* pms)
{
	ms_enclave_get_gb_t* ms = SGX_CAST(ms_enclave_get_gb_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	sgx_ec256_public_t* _tmp_p_gb_out = ms->ms_p_gb_out;
	size_t _len_p_gb_out = sizeof(*_tmp_p_gb_out);
	sgx_ec256_public_t* _in_p_gb_out = NULL;

	CHECK_REF_POINTER(pms, sizeof(ms_enclave_get_gb_t));
	CHECK_UNIQUE_POINTER(_tmp_p_gb_out, _len_p_gb_out);
	...
	ms->ms_retval = enclave_get_gb(_in_p_gb_out);
err:
	if (_in_p_gb_out) {
		memcpy(_tmp_p_gb_out, _in_p_gb_out, _len_p_gb_out);
		free(_in_p_gb_out);
...
static sgx_status_t SGX_CDECL sgx_enclave_get_sign_gb_ga(void* pms)
{
	ms_enclave_get_sign_gb_ga_t* ms = SGX_CAST(ms_enclave_get_sign_gb_ga_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	sgx_ec256_signature_t* _tmp_p_sign_gb_ga = ms->ms_p_sign_gb_ga;
	size_t _len_p_sign_gb_ga = sizeof(*_tmp_p_sign_gb_ga);
	sgx_ec256_signature_t* _in_p_sign_gb_ga = NULL;

	CHECK_REF_POINTER(pms, sizeof(ms_enclave_get_sign_gb_ga_t));
	CHECK_UNIQUE_POINTER(_tmp_p_sign_gb_ga, _len_p_sign_gb_ga);

	if (_tmp_p_sign_gb_ga != NULL) {
		if ((_in_p_sign_gb_ga = (sgx_ec256_signature_t*)malloc(_len_p_sign_gb_ga)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_p_sign_gb_ga, 0, _len_p_sign_gb_ga);
	}
	ms->ms_retval = enclave_get_sign_gb_ga(_in_p_sign_gb_ga);
err:
	if (_in_p_sign_gb_ga) {
		memcpy(_tmp_p_sign_gb_ga, _in_p_sign_gb_ga, _len_p_sign_gb_ga);
		free(_in_p_sign_gb_ga);
	}

	return status;
}

static sgx_status_t SGX_CDECL sgx_enclave_compute_msg2_mac(void* pms)
{
	ms_enclave_compute_msg2_mac_t* ms = SGX_CAST(ms_enclave_compute_msg2_mac_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	uint8_t* _tmp_p_src = ms->ms_p_src;
	size_t _tmp_len = ms->ms_len;
	size_t _len_p_src = _tmp_len;
	uint8_t* _in_p_src = NULL;
	sgx_cmac_128bit_key_t* _tmp_p_mac_out = ms->ms_p_mac_out;
	size_t _len_p_mac_out = sizeof(*_tmp_p_mac_out);
	sgx_cmac_128bit_key_t* _in_p_mac_out = NULL;

	CHECK_REF_POINTER(pms, sizeof(ms_enclave_compute_msg2_mac_t));
	CHECK_UNIQUE_POINTER(_tmp_p_src, _len_p_src);
	CHECK_UNIQUE_POINTER(_tmp_p_mac_out, _len_p_mac_out);
	...
	if (_tmp_p_mac_out != NULL) {
		if ((_in_p_mac_out = (sgx_cmac_128bit_key_t*)malloc(_len_p_mac_out)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_p_mac_out, 0, _len_p_mac_out);
	}
	ms->ms_retval = enclave_compute_msg2_mac(_in_p_src, _tmp_len, _in_p_mac_out);
err:
	if (_in_p_src) free(_in_p_src);
	if (_in_p_mac_out) {
		memcpy(_tmp_p_mac_out, _in_p_mac_out, _len_p_mac_out);
		free(_in_p_mac_out);
	}
...
static sgx_status_t SGX_CDECL sgx_enclave_encrypt(void* pms)
{
	...
	uint8_t* _tmp_buffer = ms->ms_buffer;
	size_t _tmp_buffer_len = ms->ms_buffer_len;
	size_t _len_buffer = _tmp_buffer_len;
	...
	ms->ms_retval = enclave_encrypt(_in_data, _tmp_data_len, _in_buffer, _tmp_buffer_len);
err:
...
SGX_EXTERNC const struct {
	...
	10,
	{
		...
		{(void*)(uintptr_t)sgx_enclave_set_ga, 0},
		{(void*)(uintptr_t)sgx_enclave_derive_keys, 0},
		{(void*)(uintptr_t)sgx_enclave_get_gb, 0},
		{(void*)(uintptr_t)sgx_enclave_get_sign_gb_ga, 0},
		{(void*)(uintptr_t)sgx_enclave_compute_msg2_mac, 0},
...
sgx_status_t SGX_CDECL print_byte_array(void* mem, size_t len)
{
	sgx_status_t status = SGX_SUCCESS;
	size_t _len_mem = len;

	ms_print_byte_array_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_print_byte_array_t);
	void *__tmp = NULL;

	ocalloc_size += (mem != NULL && sgx_is_within_enclave(mem, _len_mem)) ? _len_mem : 0;
	...
	ms = (ms_print_byte_array_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_print_byte_array_t));

	if (mem != NULL && sgx_is_within_enclave(mem, _len_mem)) {
		ms->ms_mem = (void*)__tmp;
		__tmp = (void *)((size_t)__tmp + _len_mem);
		memcpy(ms->ms_mem, mem, _len_mem);
	} else if (mem == NULL) {
		ms->ms_mem = NULL;
	} else {
		sgx_ocfree();
